...
# `docs-site-enterprise/preview.sh` so that editors can run
# `./docs-site-enterprise/preview.sh` after changing markdown and see the result
...
#   5. Symlinks the current repo's `docs-site-enterprise/` into the cache's
...
# Locate the docs-site-enterprise directory we're previewing.
repo_root="$(git rev-parse --show-toplevel)"
docs_site="$repo_root/docs-site-enterprise"
if [ ! -d "$docs_site" ]; then
...
# Replace the cached docs/<source>/ with a symlink to our docs-site-enterprise/
