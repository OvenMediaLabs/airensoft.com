...
These concerns can be addressed by seamlessly integrating [OvenStudio LLHLS](https://aws.amazon.com/marketplace/pp/prodview-66xy6t4xk56xs) (now OvenMediaEngine Enterprise on AWS) with **AWS CloudFront **(CDN)** **and **Signed URL**. While traditionally, connecting a streaming server with a CDN requires complexity and specialized knowledge, using [OvenStudio LLHLS](https://aws.amazon.com/marketplace/pp/prodview-66xy6t4xk56xs) (now OvenMediaEngine Enterprise on AWS) automates the entire process.


In this blog post, we provide step-by-step instructions on how to effortlessly set up integration with AWS CloudFront in just a few clicks. We also guide you on utilizing features such as the Signed URL, allowing you to share or restrict access as needed. Explore how [OvenStudio LLHLS](https://aws.amazon.com/marketplace/pp/prodview-66xy6t4xk56xs) (now OvenMediaEngine Enterprise on AWS) simplifies the integration with AWS CloudFront, making it a breeze.
...
In our upcoming blog post, we provide a step-by-step guide on effortlessly integrating [OvenStudio LLHLS](https://aws.amazon.com/marketplace/pp/prodview-66xy6t4xk56xs) (now OvenMediaEngine Enterprise on AWS) with AWS CloudFront. Whether you’re looking to enhance your streaming delivery or explore advanced features, [OvenStudio LLHLS](https://aws.amazon.com/marketplace/pp/prodview-66xy6t4xk56xs) (now OvenMediaEngine Enterprise on AWS) makes the process user-friendly, saving you time and effort. Stay tuned for our comprehensive guide, and feel free to reach out with any further inquiries.

*※ Note: This guide will be updated to accommodate integration with other CDNs besides AWS CloudFront in the future.*

## What is a Signed URL?

A Signed URL in [OvenStudio LLHLS](https://aws.amazon.com/marketplace/pp/prodview-66xy6t4xk56xs) (now OvenMediaEngine Enterprise on AWS) encrypts the URL used for media transmission between the streaming server and the viewer during streaming. This URL is valid for a specific period, and only authenticated users with the appropriate permissions can access the streaming via this URL.
...
[OvenStudio LLHLS](https://aws.amazon.com/marketplace/pp/prodview-66xy6t4xk56xs) (now OvenMediaEngine Enterprise on AWS) makes it incredibly simple to configure and use the Signed URL feature with AWS CloudFront. Follow these steps:
...
You can utilize an API to apply detailed options for individual streaming units with the Signed URL —For more information, please refer to the [**AccessToken**](https://ovenmedia.com/docs/ome-enterprise/features/rest-api#managers) section of the OvenMediaEngine Enterprise on AWS documentation. :
...
The Signed URL API is straightforwardly configured — In the OvenStudio LLHLS’s User Guide, detailed content is covered in the section labeled [**Configuration** of SignedPolicy](https://ovenmedia.com/docs/ome-enterprise/features/access-control-and-security/signedpolicy#configuration).”
...
### [Examples](https://ovenmedia.com/docs/ome-enterprise/features/access-control-and-security/signedpolicy#usage-examples):
...
You can find comprehensive details in the OvenStudio LLHLS’s User Guide under the [**SignedPolicy**](https://ovenmedia.com/docs/ome-enterprise/features/access-control-and-security/signedpolicy) section.

## Conclusion

[OvenStudio LLHLS](https://aws.amazon.com/marketplace/pp/prodview-66xy6t4xk56xs) (now OvenMediaEngine Enterprise on AWS) minimizes cumbersome processes to alleviate your concerns. If you need to distribute your streaming to a wider audience, simply integrate with AWS CloudFront with just a few clicks — [OvenStudio LLHLS](https://aws.amazon.com/marketplace/pp/prodview-66xy6t4xk56xs) (now OvenMediaEngine Enterprise on AWS) makes it possible.
...
Always, [OvenStudio LLHLS](https://aws.amazon.com/marketplace/pp/prodview-66xy6t4xk56xs) (now OvenMediaEngine Enterprise on AWS) offers the easiest and most effective options to support seamless, secure, and low-latency streaming in your service — Elevate your service with the easiest and most effective options, enabling larger audiences to enjoy a seamless, secure, and low-latency streaming experience. Join us in the future of low-latency streaming with [OvenStudio LLHLS](https://aws.amazon.com/marketplace/pp/prodview-66xy6t4xk56xs) (now OvenMediaEngine Enterprise on AWS).
